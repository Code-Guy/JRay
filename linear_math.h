...
// #define FW_ASSERT(X) FW_ASSERT(X) ((X) ? ((void)0) : FW::fail("Assertion failed!\n%s:%d\n%s", __FILE__, __LINE__, #X)) in DEBUG
...
struct Mat3
{
	...
	__host__ __device__ Mat3(Vec3f r0, Vec3f r1, Vec3f r2) { r[0] = r0; r[1] = r1; r[2] = r2; }
	
	__host__ __device__ Vec3f operator *(const Vec3f &v)
	{
		return Vec3f(r[0].x * v.x + r[0].y * v.y + r[0].z * v.z,
